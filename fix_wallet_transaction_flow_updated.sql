...
BEGIN
    ...
    LOOP
        ...
        WITH funding AS (
            SELECT COALESCE(SUM(amount), 0) as total_funding
        ...
        SELECT
            CASE
                WHEN EXISTS (SELECT 1 FROM public.wallets WHERE user_id = user_rec.user_id) THEN
                    UPDATE public.wallets
                    SET balance = (SELECT total_funding FROM funding) - 
                                  (SELECT total_payments FROM payments) +
                                  (SELECT total_refunds FROM refunds),
                        last_updated = now()
                    WHERE user_id = user_rec.user_id
                ELSE
                    INSERT INTO public.wallets (user_id, balance, currency)
                    VALUES (
                        user_rec.user_id,
                        (SELECT total_funding FROM funding) - 
                        (SELECT total_payments FROM payments) +
                        (SELECT total_refunds FROM refunds),
                        'NGN'
                    )
        ...
        RAISE NOTICE 'Synced wallet balance for user %', user_rec.user_id;
    END LOOP;
